...
from sam2.modeling.sam2_base_geosam2 import NO_OBJ_SCORE, SAM2Base
from sam2.utils.misc import concat_points, fill_holes_in_mask_scores, load_video_frame_geosam2
...
class SAM2VideoPredictor(SAM2Base):
    ...
    def init_state(
        ...
        inference_state = {}
        images, pos_maps, norm_maps, video_height, video_width = load_video_frame_geosam2(
        ...
        inference_state["pos_maps"] = pos_maps

        inference_state["norm_maps"] = norm_maps
        inference_state["num_frames"] = len(images) + 1
    ...
    def propagate_in_video(
        ...
        processing_order = (
            list(range(start_frame_idx, end_frame_idx + 1))
            + list(range(0, start_frame_idx))
        )
    ...
    def _get_image_feature(self, inference_state, frame_idx, batch_size):
        ...
        if backbone_out is None:
            ...
            pos_map = inference_state["pos_maps"][frame_idx].to(device).float().unsqueeze(0) #.unsqueeze(0)
            norm_map = inference_state["norm_maps"][frame_idx].to(device).float().unsqueeze(0)

            backbone_out = self.forward_image(image, pos_map, norm_map)
    ...
    def _run_single_frame_inference(
        ...
        (
            ...
            feat_sizes,
        ) = self._get_image_feature(inference_state, frame_idx_real, batch_size)
    ...
    def _run_memory_encoder(
        ...
        _, _, current_vision_feats, _, feat_sizes = self._get_image_feature(
            inference_state, frame_idx_real, batch_size
        )
    ...
    def propagate_in_video_v2(
        ...
        """Propagate the input points/mask across frames using a rotated processing
        order so that the seed view is processed first, followed by the rest of the
        views in a circular order. ``frame_idx_real`` is the position in the original
        video, while ``frame_idx`` is the rotated index used to pick image features.
        """
        ...
        processing_order = (
            [start_frame_idx]
            + list(range(start_frame_idx, end_frame_idx))
            + list(range(0, start_frame_idx))
        )

        for frame_idx_, frame_idx_real in tqdm(enumerate(processing_order), desc="propagate in video"):
            frame_idx = (frame_idx_ + start_frame_idx) % 13
            pred_masks_per_obj = [None] * batch_size
            for obj_idx in range(batch_size):
                obj_output_dict = inference_state["output_dict_per_obj"][obj_idx]
                # We skip those frames already in consolidated outputs (these are frames
            ...
            yield frame_idx_real, obj_ids, video_res_masks
